<template>
    <div class="container article-wrapper mt-5">
      <h1 class="text-center">{{getCategory(this.$route.params['id'])}}</h1>
      <div class="mx-auto">
  ...
  export default {
    ...
    async mounted() {
      await this.fetchArticleCategory(this.$route.params['id']);
    ...
    methods: {
      async fetchArticleCategory(id) {
          ...
          const response = await fetch(`/backend/api/article/category/${id}`, {
              method: "GET",
              mode: "cors",
              headers: {
